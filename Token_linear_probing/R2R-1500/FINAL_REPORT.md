...
> Dataset: R2R_video_1500 (4 conditions × 6000 videos), MCQ: R2R_4way_1500_json (8-way)
...
6. **Vanilla LLaVA는 vision info가 layer 1부터 last token에 흐르지만 direction → letter 매핑을 못 함.** L28에서 direction probe 0.54, letter probe 0.25 (chance 0.125). 즉 "방향은 아는데 답변 letter를 못 고름".

7. **Baseline (LoRA only) changes projector by just 1–2% in weight, nearly matches vanilla in representation**, but significantly improves LLM deep-layer direction retention. L28 letter probe 0.473 (+21%p over vanilla).

8. **Our methods (delta_direct, channel_gate) apply a ~5% projector weight delta** (row cosine > 0.98 — 방향은 보존, 스케일만 미세 조정). Direction encoding strength, linear probe acc, 그리고 특히 **last-layer letter mapping이 크게 향상** — delta_direct letter L28 = 0.528 (+28%p over vanilla, +5.5%p over baseline).
...
- **Candidates: 8 options** (4 cardinal + 4 diagonal: Up, Down, Left, Right, Up-Left, Up-Right, Down-Left, Down-Right)
- Candidates shuffled per video; answer는 "A"–"H" 중 하나
- GT direction은 4-way (up/down/left/right); diagonal은 distractor
...
| **GT Letter** | MCQ에서 정답 option의 위치 index (0=A … 7=H). Candidates가 shuffled 되어 있으므로 "direction"과 1:1 매칭이 아님 |
...
- 8-way MCQ (4 cardinal + 4 diagonal distractors)
- Candidates shuffled per video
...
- **Direction** (4-way: up/down/left/right)
- **GT Letter** (8-way: 0=A … 7=H, since options are shuffled 8-way)

Object probing omitted for this section (focus on direction↔answer pipeline).

## H.2 결과 (obj_place, N = 2000)
...
| 1 | 0.512 | 0.514 | 0.570 | **0.575** |
| 4 | 0.683 | 0.697 | 0.717 | **0.742** |
| 8 | 0.656 | 0.703 | 0.733 | **0.756** |
| 12 | 0.679 | **0.802** | 0.783 | 0.790 |
| 16 | 0.622 | **0.838** | 0.798 | 0.810 |
| 20 | 0.635 | 0.853 | 0.864 | **0.873** |
| 24 | 0.603 | 0.863 | **0.868** | 0.866 |
| **28 (last)** | **0.536** | 0.836 | 0.845 | **0.848** |

### GT Letter probe (8-way, chance = 0.125)

| Layer | Vanilla | Baseline | Delta_direct | Channel_gate |
|---|---|---|---|---|
| 0 (embed) | 0.136 | 0.136 | 0.136 | 0.136 |
| 1 | 0.247 | 0.238 | 0.245 | 0.250 |
| 4 | 0.239 | 0.242 | 0.247 | 0.239 |
| 8 | 0.232 | 0.222 | 0.235 | 0.230 |
| 12 | 0.239 | 0.248 | 0.244 | 0.240 |
| 16 | 0.222 | 0.245 | 0.237 | 0.242 |
| 20 | 0.247 | 0.246 | 0.269 | 0.271 |
| 24 | 0.236 | 0.285 | 0.287 | 0.262 |
| **28 (last)** | **0.245** | **0.473** | **0.528** | **0.468** |

## H.3 해석 — 이 분석의 하이라이트

### 1) Layer 0 (embedding): 모두 chance
- 400 videos 전부에 대해 prompt가 동일 (candidates가 shuffled된 것 외에는)
- Last token은 아직 self-attention이 안 일어났으므로 vision info 흡수 안 됨

### 2) Layer 1–4: Direction 정보 급속 흡수
- Vanilla: chance → 0.51 → 0.68 (L4 peak)
- **Ours가 얕은 layer에서 이미 우위**: L4에서 vanilla 0.68, channel_gate 0.74 (+6%p)
- Fine-tuning이 얕은 attention에서부터 direction info를 더 효율적으로 aggregate

### 3) Layer 8–28: 핵심 현상
| 관점 | Vanilla | Baseline | Delta_direct | Channel_gate |
|---|---|---|---|---|
| **Direction L28** | 0.536 (↓ from peak) | 0.836 | 0.845 | 0.848 |
| **Direction 깊은 layer 유지** | 급격 감쇠 | 강 보존 | 강 보존 | 강 보존 |
| **GT Letter L28** | 0.245 (≈ chance × 2) | 0.473 | **0.528** | 0.468 |

#### 3a) Direction은 3 모델 모두 깊게 유지됨 (Baseline ≈ Ours)
- Baseline도 L12 이후 direction probe 0.80–0.86로 vanilla (0.53)보다 훨씬 높음
...
#### 3b) GT Letter는 오직 Layer 28에서 크게 상승
- L0–L24: 모든 모델이 0.22–0.29 수준 (chance 0.125 대비 약간만)
- **L28에서만** trained 3개가 0.47–0.53으로 점프
- **Vanilla는 L28에서도 0.245로 chance 근방** ← 이것이 vanilla가 MCQ를 못 푸는 이유

### 4) Information–Generation Gap

**Vanilla**:
- Direction probe L4–L12: 0.65–0.68 (direction 정보 보유)
- Letter probe L28: 0.245 (정답 letter 출력 못 함)
- **즉 "방향은 알지만 answer letter로 변환을 못 함"**

**Trained**:
- Direction probe L28: 0.84–0.85 (direction 정보 강 보존)
- Letter probe L28: 0.47–0.53 (letter 출력 능력 획득)
- Training의 진짜 효과 = **representation → generation format mapping** 학습

### 5) Delta_direct가 letter mapping 최강 (0.528)

- baseline 0.473 → delta_direct 0.528 (+5.5%p)
- **Delta direction regression loss**가 LLM에 "direction 정보를 answer token 형태로 출력하는 방법"을 가장 잘 가르침
- Channel_gate는 direction 정보 자체는 가장 강하지만 (L20 0.873), letter mapping은 baseline과 비슷 — **explicit direction supervision이 letter head 학습에 더 직접적**

### 6) Q7–Q9 답

- **Q7**: LLM은 layer 1부터 vision 정보 흡수, peak는 vanilla = L4, trained = L20–24 (layer 위치 뒤로 이동)
- **Q8**: Vanilla vs trained 격차는 L12 이후 direction probe에서, L28 letter probe에서 각각 드러남
- **Q9**: **직접 NO** — direction 정보를 가진다고 letter를 맞추는 건 아니다. Letter mapping은 explicit training (LoRA + 특히 direction loss)로만 얻어짐
...
| **Generation** | **Letter probe L28 = 0.245 (≈ chance)** | H |
...
| LLM deep-layer direction | **L28 probe 0.84** (vanilla 0.54) | H |
| **Generation** | **Letter L28 = 0.473** (+21%p over vanilla) | H |
...
| LLM direction at peak | L20 = 0.87 (ours) vs 0.85 (baseline) | 작은 상승 |
| **GT letter L28 (delta_direct)** | **0.528** | **+5.5%p** over baseline |
...
- **Output side (LM head 근처)에서 explicit supervision**이 generation 성능에 직접 기여 (Letter mapping은 오직 L28에서만 대폭 향상)
...
- Probes (`probe_llm_hidden.py`):
  ...
  - GT Letter: `candidates.index(dir_text_map[direction])` — 8-way index
